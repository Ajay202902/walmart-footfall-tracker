...
def femaleVisitors():
    ...
    sql = text('SELECT COUNT(id) AS female_visitor FROM visitors_log WHERE date = "'+currentDate+'" AND gender = 2')
    result = db.engine.execute(sql)
    rawdata = result.fetchall()
    jsondata = jsonify([dict(row) for row in rawdata])
    return jsondata
...
def ageGroupClassfication():
    ...
    for a in range(1,6):
        ...
        for g in range(1,3):
            ...
            # Today's Visitors
            sqlDvisitors = text('SELECT COUNT(id) AS age_today_visitors FROM visitors_log WHERE date = "'+currentDate+'" AND gender = "'+str(g)+'" AND age = "'+str(a)+'"')
            ...
            # Overall Visitors
            sqlOverallVisitors = text('SELECT COUNT(id) AS age_overall_visitor FROM visitors_log WHERE gender = "'+str(g)+'" AND age = "'+str(a)+'"')
            resultOverall = db.engine.execute(sqlOverallVisitors)
            associtationOverallData = resultOverall.fetchall()
            ageGroupOverall = associtationOverallData[0].age_overall_visitor  
